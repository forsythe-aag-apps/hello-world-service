        ...
        if (!pullRequest) {
            container('kubectl') {
                timeout(time: 3, unit: 'MINUTES') {
                    rocketSend channel: 'jenkins', message: "@here ${serviceName} - waiting approval. [Click here](https://${env.JENKINS_URL}/blue/organizations/jenkins/greetings-service/detail/master/${env.BUILD_NUMBER}/pipeline)", rawMessage: true
